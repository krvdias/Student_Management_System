...
import { SubjectForm, SubjectData } from '@/constants/types';
import { addSubject, deleteSubject, editSubject, searchSubject } from '@/service/adminRoutes';
import { toast } from 'react-toastify';
import EditSubjectModel from './SubjectEdit';
...
function SubjectComponent() {
  ...
  const [editFormData, setEditFormData] = useState<SubjectForm>({
      subjectId: "",
      name: "",
  });
  ...
  const fetchSubjects = async (term: string = '') => {
      try {
          setLoading(true);
          const response = await searchSubject({ searchTerm: term });
  ...
  const handleEditClick = (sub: SubjectData) => {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
      ...
      try {
          if (selectedSubject) {
              const response = await editSubject(editFormData, selectedSubject.id);
              if (response.data.success) {
  ...
  const renderDeleteConfirm = () => (
      <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-3xl shadow-lg w-full max-w-sm">
              ...
              <p className="mb-6">Do you want to delete this subject?</p>
                  ...
                  <div className='overflow-x-auto'>
                      <table className='min-w-full divide-y divide-gray-200'>
                          <thead className='bg-yellow-400/70'>
                              <tr>
                                  <th className='px-6 py-3 text-center text-sm font-medium text-black tracking-wider'>Subject ID</th>
                                  <th className='px-6 py-3 text-center text-sm font-medium text-black tracking-wider'>Subject Name</th>
                              ...
                              {loading ? (
                                  ...
                                  paginatedSubjects.map((sub) => (
                                      <tr key={sub.id} className='hover:bg-gray-50'>
                                          <td className='px-6 py-4 text-center whitespace-nowrap text-sm text-gray-900'>{sub.subject_id}</td>
              ...
              {subjects.length > 0 && (
                  ...
                  <div>
                      ...
                      <span className='font-medium'>
                          {Math.min(currentPage * itemsPerPage, subjects.length)}
                      </span>{' '}
                      of <span className='font-medium'>{subjects.length}</span> results
                      </p>
                  </div>
