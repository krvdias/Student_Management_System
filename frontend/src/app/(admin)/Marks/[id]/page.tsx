...
import { addMark, deleteMark, editMark, getMarks } from '@/service/adminRoutes'
...
function Page() {
  ...
  const fetchMarks = async () => {
    try {
      setLoading(true);
      const response = await getMarks(requestData);
    ...
    try {
      if (isEditMode && selectedMark) {
        ...
        if (response.data.success) {
          ...
          toast.error(response.data.message || 'Failed to add Mark');
        }
      }
      setIsAddMode(false);
      setIsEditMode(false);
  ...
  const handleHistory = () => {
    if (id) {
      ...
      router.push(`/Marks/${id}/History?${params.toString()}`);
    }
  };
      ...
      <div className='flex justify-between items-center gap-2 mb-2'>
        ...
        <div className="relative flex-grow flex-col  max-w-xs">
          ...
          <div className='flex flex-row justify-end gap-3'>
            <button
              className='bg-[#1346DD] text-sm text-white font-semibold px-4 py-2 rounded-lg whitespace-nowrap hover:bg-blue-700 transition shadow-lg mt-4 inline-flex items-center'
